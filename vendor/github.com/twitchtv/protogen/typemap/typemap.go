...
func (r *Registry) FieldComments(md *MessageDefinition, field *descriptor.FieldDescriptorProto) (DefinitionComments, error) {
	for i, f := range md.Descriptor.Field {
		if f == field {
			path := md.path
			path = append(path, messageFieldPath, int32(i))
			return commentsAtPath(path, md.File), nil
		}
	}
	return DefinitionComments{}, errors.Errorf("field not found in message")
}
